...
def load_memory(folder_name: str, num_rounds: int = 50,
    ...
    - 对话轮数未超出 num_rounds 且未强制使用概括 → 返回原始对话
    - 超出 num_rounds 或 force_use_summary=True 时，用概括替代较早的轮次
    """
    ...
    # 合并所有概括为一条上下文
    summary_parts = [s["content"] for s in summaries]
    combined = "\n\n---\n\n".join(summary_parts)

    summary_msg = {
        "role": "system",
        "content": "以下是之前对话的概括记录，供你参考上下文：\n\n" + combined
    }
...
def save_summary(folder_name: str, summary_text: str):
    """将新的概括结果存入 memory.json, 更新标记, 必要时触发合并"""
    mem_cfg = load_memory_config(folder_name)
    summaries = mem_cfg.get("summaries", [])
